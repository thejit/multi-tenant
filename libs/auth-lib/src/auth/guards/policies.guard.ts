...
export class PoliciesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    try {
      const isAuthUser = this.checkPolicies(
        user['policies'].map((policy) => JSON.parse(policy)),
        policies,
      );
      return isAuthUser;
    } catch (err) {
  ...
  private checkPolicies(userPolicies: Policy[], policies: Policy[]) {
    const isAuthUser = policies.filter((resourcePolicy) => {
      return (
        userPolicies.filter((policy) =>
          this.isAutherize(resourcePolicy, policy),
        ).length >= 1
      );
    }).length;
    return isAuthUser >= 1;
  ...
  private isAutherize(resourcePolicy: Policy, policy: Policy): unknown {
    return (
      this.hasResource(resourcePolicy, policy.resources) &&
      this.hasAction(resourcePolicy, policy)
    );
  }
  ...
  private hasAction(resourcePolicy: Policy, policy: Policy): unknown {
    return (
      resourcePolicy.action.filter((action) => !policy.action.includes(action))
        .length == 0
    );
  }
}
